...
The donut chart on the left shows the total area of all the different landcovers found a 2018 analyis for the Lez River Basin. Cropland is roughly **14%** of the basin, while meadow, urban, forest, and water surfaces make up most of the rest. The pie on the right focuses only on crops, showing each crop's share of the part of the basin that is cropland. Vineyards dominate cropland, with cereals a distant second (Theia 2018). 

**Note on the numbers.** Per-crop *areas* shown here come from the full-basin Theia OSO 2018 land-cover product. Per-crop *water use* (the charts in this story) currently reflects only the NW quarter of the basin; pyWaPOR processing for the other three quarters is in progress, and these figures will be replaced with full-basin values when that completes. Orchards (class 14) are not parcel-level classified; for this analysis we treat them as ~60% olives / ~40% stone fruit (see Methods in the About section for more information on our assumptions).

---
section: 02b
label: Finding
title: Crops use water differently (Intensity per hectare)
visual: per_crop_aeti_chart
visual_type: chart
---

How much water does each crop consume per square metre of cropland over the season? `mm/season` is equivalent to **litres of water per m² of cropland**. Orchards and grasslands consume the most per area, vineyards are next, cereals consume the least. But how much water do these crops consume overall?
...
Most cropland evapotranspiration comes from rainfall (**green water**). The portion drawn from irrigation (**blue water**) is what actually competes with Montpellier's drinking-water supply, since both come either from from the same karst-fed system, or indicate other, potentially unsustainable, groundwater extraction. Vineyards are by far the largest source of blue-water demand because of their dominant area; cereals are nearly entirely rainfed.

*Caveat: the GEOS-5 precipitation forcing used here overestimates Mediterranean rainfall, which inflates the green-water bars and depresses the blue-water bars. The blue-water totals are likely under-estimates; switching to a CHIRPS or in-situ precipitation record is a recommended enhancement.*
...
{{ findings.year }} was one year. Climate projections for the western Mediterranean point to hotter, drier summers and more variable rainfall. The seasonal squeeze visible in this single year is likely to intensify. The questions for stakeholders are not just "how much water" but "when, for whom, and at what tradeoff."
